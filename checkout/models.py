    ...
    def clean(self):
        if self.percent_off is not None and self.percent_off <= 0:
            raise ValidationError(
                {"percent_off": "Promo code must be greater than 0%."}
            )
...
class Order(models.Model):
    ...
    def _calc_delivery(self) -> Decimal:
        return (
            Decimal("0.00")
            if self.subtotal >= self.FREE_DELIVERY_THRESHOLD
            else self.DELIVERY_FLAT
        )
    ...
    def update_totals(self, persist: bool = True):
        ...
        sum_items = (
            self.items.aggregate(s=Sum("item_total"))["s"]
            or Decimal("0.00")
        )
        ...
        if self.applied_promo and self.applied_promo.is_active:
            self.discount_amount = (
                self.subtotal
                * (self.applied_promo.percent_off / Decimal("100"))
            ).quantize(
                Decimal("0.01"),
        ...
        self.total_due = max(total, Decimal("0.00")).quantize(
            Decimal("0.01")
        )
...
class OrderItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        price = (
            self.product.special_offer_price
            if getattr(self.product, "has_discount", False)
            and self.product.special_offer_price
