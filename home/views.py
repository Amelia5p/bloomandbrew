        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                if "is already a list member" in error_detail:
                    ...
                    messages.error(request, f"Mailchimp error: {error_detail}")
            else:
                messages.error(request, f"Unexpected error: {response.status_code}")
        except requests.exceptions.RequestException as e:
            print("Mailchimp connection error:", e)
            messages.error(request, "Could not connect to the mailing list service. Please try again later.")
